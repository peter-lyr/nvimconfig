...
local sdkcbp_dir = require("plenary.path"):new(vim.g.boot_lua):parent():parent():parent():parent():joinpath('opt', 'sdkcbp')

vim.g.cmake_app_py = sdkcbp_dir:joinpath('autoload', 'cmake_app.py')['filename']
vim.g.cmake_others_py = sdkcbp_dir:joinpath('autoload', 'cmake_others.py')['filename']
...
vim.keymap.set({ 'n', 'v' }, '<c-F9>', function() sdkcbp() end,  { silent = true })
